    ...
    if (request.method === 'POST') {
      try {
        ...
        try {
          ...
          // If response is not JSON, create a simple success message
          result = { ok: true, message: 'Data forwarded to Vercel', status: response.status };
        }
        
        // Return response immediately with CORS headers
        // This ensures ESP32 gets a quick response
        return new Response(JSON.stringify(result), {
          status: 200, // Always return 200 to ESP32 for faster response
          headers: {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods': 'POST, GET, OPTIONS',
